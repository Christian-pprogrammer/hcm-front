...
const DeleteAcc = ({ showModal, onClose, id }: { showModal: Boolean, onClose: () => void, id: string }) => {
  ...
  const handleDeleteSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        setLoading(true)
        const resdelete = await groupService.deleteGroup(id);
        setAlertData({
          alert: true,
          ...
          <form method="post" onSubmit={handleDeleteSubmit}>
            <div className="modal-bod py-2">
              {/* <div className='py-1'>
                ...
                <input value={password} onChange={(e) => setPassword(e.target.value)} className="shadow appearance-none bg-inputG border rounded w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="password" placeholder="Password" />
            ...
            <div className="modal-footer flex py-2 justify-between">
              ...
              <button type="submit" className="btn ripple bg-backG text-white py-2 px-6 lg:px-6 lg:py-2 rounded-sm shadow-lg  btn-secondary" data-dismiss="modal">Delete</button>
            </div>
          </form>
        </div>
      </div>
