...
const Header = () => {
    ...
    const trl = useTranslations('Homepage.Navbar');
        ...
        <header className={` ${ShowLinks && 'flex-col gap-6 origin-center duration-300 '} z-10 fixed top-0 left-0 right-0 bg-white duration-500 justify-between drop-shadow-xl px-2 flex md:px-2 lg:px-[4em]  min-h-[10vh]`}>
            ...
            <div className={`flex ${ShowLinks && 'flex flex-col  gap-6 justify-center'}  place-items-center justify-center  md:gap-[2em] lg:gap-[8em]`}>
                <ul className={`text-black text-[14px] font-normal ${ShowLinks ? 'flex flex-col gap-6 text-center justify-center' : 'hidden'}  md:flex gap-10`}>
                    <li className={`${ChangeHeader('/')} hover:border-b-2 hover:border-solid hover:border-backG py-2 hover:duration-700 hover:text-blue-500`}><Link href="/">{trl(`links.homedata`)}</Link></li>
                    <li className={`${ChangeHeader('/#about')} hover:border-b-2 hover:border-solid hover:border-backG py-2 hover:duration-700 hover:text-blue-500`}><Link href="/#about">{trl(`links.about`)}</Link></li>
                    <li className={`${ChangeHeader('/#services')} hover:border-b-2 hover:border-solid hover:border-backG py-2 hover:duration-700 hover:text-blue-500`}><Link href="/#services">{trl(`links.showcase`)}</Link></li>
                    <li className={`${ChangeHeader('/patient/landing-appointment-page')} hover:border-b-2 hover:border-solid hover:border-backG py-2 hover:duration-700 hover:text-blue-500`}><Link href="/patient/landing-appointment-page">{trl(`links.appointment`)}</Link></li>
                </ul>
