...
const BasicScheduleInfo = ({ FormData, setFormData }: { FormData: ISchedule, setFormData: React.Dispatch<React.SetStateAction<ISchedule>> }) => {
    ...
    return (
        <>
            ...
            <div className="py-1">
                ...
                <select value={FormData?.service_id} onChange={(e) => setFormData({ ...FormData, service_id: e.target.value })} className="shadow hover:border-solid hover:border-2 duration-500 rounded-md hover:border-backG border-2 border-white appearance-none bg-inputG w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Select the status">
                    <option value={''}>{'Select Service'}</option>
                    {servicesArr && servicesArr.map((option: IService) => (
                        <option key={option.service_id} value={option.service_id}>{option.service}</option>
                    ))}
                </select>
            </div>
            {errors.length > 0 && (
