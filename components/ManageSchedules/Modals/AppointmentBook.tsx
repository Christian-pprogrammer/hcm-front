import React, { useState, useEffect } from 'react';
import * as ReactDOM from 'react-dom';
import appointmentService from '../../../services/appointments/appointment.service';
import { notifyError, notifySuccess } from '../../alert';
import { PayAppointment, PayAppointmentImpl } from '../../../utils/ModalTypes';
import LoaderCache from '../../loaders/LoaderCache';

const AppointmentBook = ({ MapModal, onClose, appointmentId }: { MapModal: Boolean, onClose: any, appointmentId: string }) => {
  const errors: string[] = [];

    const [isBrowser, setBrowser] = useState<Boolean>(false);
    const [loading, setLoading] = useState(false);
    const [FormData, setFormData] = useState<PayAppointment>(PayAppointmentImpl);
    const [intervalId, setIntervalId] = useState<NodeJS.Timeout | null>(null);

    if (!FormData.phoneNumber) {
      errors.push("Please provide the number to process appointment payment!");
    }
    FormData.appointmentId = appointmentId;

    useEffect(() => {
        setBrowser(true)
    }, []);

    const handleClose = () => {
      // Clear the interval if it's set
      if (intervalId) clearInterval(intervalId);
        onClose()
    }

    const handleSubmit = async (e: any) => {
        e.preventDefault();
        try {
          setLoading(true);
          const paymentCreds = await appointmentService.payForAppointment(FormData);

          if (paymentCreds.status === 200) {
            const id = setInterval(async () => {
              try {
                const res = await appointmentService.getPaymentStatus(appointmentId);
                // Check if response is FAILED or SUCCESSFUL
                if (res.data === 'FAILED') {
                  clearInterval(id);
                  setLoading(false); // Stop loading spinner
                  notifyError("Booking appointment failed, try again!");
                  handleClose;
                }
                if (res.data === 'SUCCESSFUL') {
                  clearInterval(id);
                  setLoading(false); // Stop loading spinner
                  notifySuccess("Successfully booked an appointment.");
                  handleClose;
                }
              } catch (error) {
                console.error('Error while checking payment status, try again');
                clearInterval(id);
                setLoading(false);
              }
            }, 30000); // 30 seconds in milliseconds
            // Save interval ID to state
            setIntervalId(id);
          } else {
            notifyError("Booking appointment failed, try again!");
            handleClose;
          }
      } catch (error: any) {
          const ERROR_MESSAGE = error.response ? error.response?.data?.error || "Appointment booking failed, try again!" : error.error;
          notifyError(ERROR_MESSAGE);
      }
      onClose();
    }


    const ModalContent = MapModal ? (
        <div className="modal-portal bg-modalG h-screen w-screen px-5 flex place-items-center z-20 absolute top-0 bottom-0 left-0 right-0 justify-center">
          {loading ? <LoaderCache/> :
            <div className="modal px-10 py-4 bg-white rounded-sm drop-shadow w-full md:w-1/2 lg:w-[30vw]">
                <div className="modal-content">
                    <div className="modal-header py-2 flex justify-between">
                        <h5 className="modal-title font-bold text-backG ">Pay from: </h5>
                    </div>
                    <div className="py-1">
                      <label className="block text-gray-700 text-sm font-bold">
                        Mobile number
                      </label>
                      <input value={FormData.phoneNumber} onChange={(e) =>  setFormData({ ...FormData, phoneNumber: e.target.value})} className="shadow hover:border-solid hover:border-2 duration-500 rounded-md hover:border-backG border-2 border-white appearance-none bg-inputG w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" placeholder="Enter number to pay from" />
                    </div>
                    {errors.length > 0 && (
                <div className='py-2'>
                    <ul>
                        {errors.map((error: string, index: number) => (
                            <li key={index} className='flex text-[10px] place-items-center gap-6 text-red-500'>
                                <span aria-hidden="true">&times;</span><span>{error}</span></li>
                        ))}
                    </ul>
                </div>
            )}
                    <form onSubmit={handleSubmit}>
                        <div className="modal-footer flex py-2 gap-2 justify-between">
                            <button type="button" className="btn bg-backG text-white py-2 px-4 lg:px-10 lg:py-3 btn-secondary" data-dismiss="modal" onClick={handleSubmit}>Pay</button>
                        </div>
                    </form>
                </div>
            </div>
                        }
        </div>
    ) : null
    if (isBrowser) {
        return ReactDOM.createPortal(ModalContent, document.getElementById('modal-root') as HTMLElement)
    } else {
        return null
    }

}

export default AppointmentBook
