...
const RemoveService = ({ showModal, onClose, id }: { showModal: Boolean, onClose: () => void, id: string }) => {
  ...
  const handleClick = async (e: any) => {
    ...
    try {
      ...
      let result = await servicesService.removeServiceFromHospital(hospitalId, id);
      if (result.status === 200) {
        notifySuccess("Successfully removed the service");
        handleClose();
      }
    } catch (error: any) {
