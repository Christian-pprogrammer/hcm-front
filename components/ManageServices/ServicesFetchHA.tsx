...
const ServicesFetchHA = ({ service, showModal }: { service: IService, showModal: boolean }) => {
        ...
        <tr key={service.service_id} className="bg-white hover:bg-inputG text-[14px] hover:cursor-pointer">
            ...
            {showModal &&
                <td className='whitespace-nowrap text-[#00000043] py-2'>
                    <button onClick={() => setshowRemoveModal(true)} className='text-backG bg-linear py-1.5 ripple hover:text-white px-6 border-2 border-backG text-[12px] rounded-md font-bold'> Remove Service </button>
                </td>
            }
            <RemoveService id={service.service_id} showModal={showRemoveModal} onClose={() => setshowRemoveModal(false)} />
        </tr>
    )
}

export default ServicesFetchHA